...
/**
 * Calculates the total number of passengers by summing the counts
 * from all passenger category input fields.
 */
function calculateTotal() {
  // 1. Define the IDs of the passenger input fields to sum up
  const passengerIds = ['seniorCitizen', 'pwd', 'adult', 'children', 'infants'];

  let totalCount = 0;

  // 2. Loop through each ID, get the value, convert to a number, and sum it
  passengerIds.forEach((id) => {
    ...
    // If the value is empty or invalid, parseInt returns NaN.
    // The || 0 ensures NaN is treated as 0, preventing calculation errors.
    const count = parseInt(valueString, 10) || 0;

    totalCount += count;
  });

  // 3. Update the 'totalPassenger' input field with the final calculated number
  document.getElementById('totalPassenger').value = totalCount;
}

// Run calculation once on load to ensure 'Total Passenger' shows '0' if 'value="0"' is missed on one of the inputs
// Although your HTML has value="0", this is good practice.
document.addEventListener('DOMContentLoaded', calculateTotal);
// document.addEventListener('DOMContentLoaded', () => {
//   function calculateTotal() {
//     const getValue = (id) => {
//       const element = document.getElementById(id);
//       return Math.max(0, parseInt(element.value) || 0);
//     };
